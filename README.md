...
└── docs/
    ...
    └── portfolio_strategy.md    # 地域別最適ポートフォリオ戦略
```
...
[地域別最適ポートフォリオ戦略](docs/portfolio_strategy.md) では、売却間隔（12h/24h/72h）ごとの最適製品構成を分析しています。

| 地域 | 12h | 24h | 72h | 支配的制約 |
|---|--:|--:|--:|---|
| 四号谷地 | +623/min | +623/min | +623/min | 取引券蓄積レート |
| 武陵 | +55.0/min | +30.6/min | +10.2/min | 貯蔵上限 |

**主な発見:**
- 四号谷地は取引券蓄積レートが律速。鉱石は余るため、カプセルⅢ/缶詰Ⅲに集中
- 武陵は貯蔵上限が長期運用の律速。息壌のみを生産し、可能な限り頻繁に売却
