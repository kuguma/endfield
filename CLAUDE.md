...
- 電力は unit/sec
- 取引価格は地域固有の取引券（券）
- 鉱石: 源石鉱 (originium)、紫晶鉱 (amethyst)、青鉄鉱 (ferrium)、赤銅鉱物 (cuprium)
- 地域: 四号谷地 (Valley IV)、武陵 (Wuling)

## バージョンアップ時の調査事項

ゲームのバージョンが上がると以下の値が変わる可能性があるため、データ更新時に必ず調査すること:

- **鉱石の最大採掘レート** — 地域開発レベル上限の引き上げにより増加する（例: v1.1で武陵の源石鉱が360→480、v1.2で赤銅鉱120→180）
- **マシンの最大設置台数** — 工業発展計画の追加により増える（例: v1.1で天有洪炉が2→4台、v1.2で4→8台 (洪炉拡張Ⅲ) → 12台 (洪炉拡張Ⅳ)）
- **拠点レベル・蓄積レート** — 新しい拠点レベルが追加される（例: v1.1で武陵拠点Lv3、蓄積レート12,900→26,880/h）
- **新拠点の追加** — マップ拡張で全く新しい拠点が増える（例: v1.2で武陵に「心臓修復施設」追加、独立した蓄積率/上限を持つ）
- **新製品・新レシピ・新マシン** — 新しい鉱石種や中間素材チェーンが追加される（例: v1.1で赤銅鉱物・壌晶チェーン、v1.2で沈殿酸・緋銅チェーン・重息壌チェーン）
- **新流体・新ポンプ** — 採掘可能な液体が増える（例: v1.2で沈殿酸 + 耐酸性液体ポンプMk II）
- **貯蓄上限** — 地域開発で変動する可能性あり (v1.1 48,000 → v1.2 50,000)
- **期間限定アイテム** — AIC イベント期間中のみ取引可能なアイテム (例: v1.2「掌中救星」イベントの息壌ひょうたん)
- **取引対象外の中間素材** — 装備生産専用などで取引券に換算されない製品もある（例: 緋銅装備部品 = Hetonite Component）
