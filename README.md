...
準拠バージョン：**v1.2「春の暁、訪れし時」(2026-04-17)**
...
武陵（Wuling）で拠点取引可能な全 **10 製品** (v1.2)。
...
[武陵 出荷製品の効率分析](docs/wuling_analysis.md) では、全製品について同様の分析を行っています。

| 指標 | 結果 |
|---|---|
| 最高効率 | 緋銅部品 / 芽針注射剤Ⅱ / 赤銅部品 (+2000% 級) |
| 利益/min最高 | 緋銅部品 (+357/min) / 中容量武陵バッテリー |
| 損益分岐点 | 錦草ソーダ / 芽針注射剤Ⅰ / 小容量バッテリー (~0%) |
| v1.2 期間限定 | 息壌ひょうたん (40 武陵券 + 10 支援成果券) |

v1.2で **緋銅部品 (48券)** と **重息壌 (27券)** が追加され、新拠点 **心臓修復施設** で売却可能に。
...
| 武陵 (v1.2) | **764.4/min** | 源石94%, 青鉄67%, 赤銅83%, 沈殿酸94% | [解](docs/optimization_solved_wuling.md) |
| 武陵 (v1.2 +ひょうたん) | 764.4/min + **支援成果券 36.8/min** | - | [解](docs/optimization_solved_wuling_with_gourd.md) |

※マシン台数0.25刻み、+30%ボーナス込み。**v1.1 (582/min) → v1.2 (764.4/min) で +31.3% 改善** (心臓修復施設の追加により拠点蓄積上限が +31% 拡大)。詳細は各解を参照。
...
- それぞれの地域において、得ることのできる鉱物資源のレートは有限
  - 最大拡張時の採掘資源レート（/min）:
    ...
    | 地域 | 源石鉱 | 紫晶鉱 | 青鉄鉱 | 赤銅鉱 | 沈殿酸 |
    |---|--:|--:|--:|--:|--:|
    | 四号谷地 | 560 | 240 | 1080 | - | - |
    | 武陵 (v1.2) | 480 | 0 | 90 | **180** | **240** |
...
- 地域の各拠点ごとに、取引券の貯蓄レートと貯蓄上限が存在
  ...
  - 各拠点の取引券蓄積レートと上限:
    ...
    | 武陵 | 天王原 (Lv3) | 26,880/h | 2,600,000 | 約97時間 |
    | 武陵 | 心臓修復施設 (Lv2) | 8,400/h | 720,000 | 約86時間 |
